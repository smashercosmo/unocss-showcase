...
it("should generate correct CSS", async () => {
  const uno = await createGenerator(configWithoutSafelist);
    ...
    <Box>
      <div>
        <Box
          flex={1}
          display="grid"
          grid-template-columns={condition1 ? "200px minmax(900px, 1fr) 100px" : (condition2 || "repeat(3, 1fr)")}
          grid-template-rows="repeat(10, 18rem)"
          grid-column="1 / -1"
          grid-column="1 / 1"
          grid-row={
...
it("should generate css variable", async () => {
  const uno = await createGenerator({...configWithoutSafelist, preflights});
  const output = await uno.generate(`
    <Box>
      Hello
    </Box>
